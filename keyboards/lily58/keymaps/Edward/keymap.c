...
bool animated_hash(int x, int y) {
    int  modulus      = 16;
    int  offset       = (timer_read() / 60000) * (modulus / 2);
    bool stripe_color = stripe(modulus - x % modulus, (y + offset) % modulus, 1) | stripe(x % modulus, (y + offset) % modulus, 1);
